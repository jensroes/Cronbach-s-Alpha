...
data {
  int<lower = 1> N;
  int<lower = 0> likert_min;
  int<lower = 0> likert_max;
  int<lower = 1> K;
  vector<lower = likert_min, upper = likert_max>[K] y[N];
}
...
parameters {
  real<lower = likert_min, upper = likert_max> mu[K];
  real<lower = 0> sigma[K];
  real<lower = likert_min * K, upper = likert_max * K> mu_x;
